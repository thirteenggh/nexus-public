...
Ext.define('NX.coreui.app.PluginStrings', {
  ...
  keys: {
    ...
    Assets_Info_Last_Downloaded: '上次下载',
    ...
    Assets_Info_Downloaded_Count: '近30天',
    Assets_Info_Downloaded_Unit: '下载',
    Assets_Info_UploadedBy: '上传者',
    Assets_Info_UploadedIp: '上传者IP',
    ...
    FolderInfo_Delete_Success: '该文件夹正在后台删除',
    Component_Asset_Tree_Title_Feature: 'Tree',
    Component_Asset_Tree_Description_Feature: '查看组件和资产的树状图',
    Component_Asset_Tree_EmptyText_View: '存储库中未找到组件或资产',
    Component_Asset_Tree_Expand_Failure: 'Unable to show requested tree entry',
    Component_Asset_Tree_Filtered_EmptyText_View: '所有组件均已被过滤，尝试使用<a href="#browse/search">搜索</a>吗？',
    Component_Asset_Tree_Results_Warning: '可能还有其他结果，如找不到需要的结果，请尝试过滤或搜索。',
    Component_Asset_Tree_Html_View: 'HTML View',
    Component_Asset_Tree_Upload_Component: '上传组件',

    ComponentDetails_Delete_Button: '删除组件',
    ComponentDetails_Analyze_Button: '分析程序',
    ComponentDetails_View_Vulnerabilities_Button: '查看漏洞',
    ComponentDetails_View_Vulnerabilities_Count_Button: '查看{0}个漏洞',
    ComponentDetails_Browse_Snapshots_Button: '浏览快照',
    ComponentDetails_Delete_Body: '将删除与该组件关联的所有资产：{0}',
    ComponentDetails_Delete_Title: '确定删除？',
    ComponentDetails_Delete_Success: '已删除组件：{0}',
    ComponentDetails_Analyze_Success: '正在分析中。报告生成后将通过电子邮件发送到邮箱。',
    ComponentDetails_Loading_Mask: '加载中...',
    ...
    ComponentUtils_Delete_Asset_No_Permissions: '您无权删除此资产',
    ComponentUtils_Delete_Component_No_Permissions: '您无权删除此组件',

    AnalyzeApplication_Button_Unauthenticated: '请先登录',

    AnalyzeApplicationWindow_Title: '分析程序',
    ...
    // Browse -> Search
    Search_Text: '搜索',
    Search_Description: '搜索组件',
    Search_SearchRestrictions: '搜索必须至少包含一个筛选、一个超出格式的附加筛选、通配符(*)前至少三个字符，并且不能以通配符(*)开头。',
    Search_KeywordSearchRestrictions: '将筛选条件写在引号中以精确搜索；否则，筛选条件将被逗号、空格、破折号或正斜杠分隔。<br>' +
                                      '所有关键字搜索都会自动在每个筛选条件末尾附加一个通配符(*)。',
    Search_SaveSearchFilter_Title: '保存搜索过滤器',
    Search_SaveSearchFilter_Name_FieldLabel: '过滤器名称',
    Search_SaveSearchFilter_Description_FieldLabel: '过滤器说明',
    Search_Results_Limit_Message: '仅显示前 {0} 个结果（共 {1} 个）',
    ...
    Search_SearchResultList_EmptyText: '没有符合筛选条件的组件',
    ...
    // Browse -> Browse
    FeatureGroups_Browse_Text: '存储库',
    FeatureGroups_Browse_Description: '浏览存储库中的组件和资产',
    ...
    Repository_Copy_URL: '使用您存储库的URL（如下所示）将其他软件包或工具连接到您的存储库。 ' +
      '更多信息，请参阅' +
      '<a href="http://links.sonatype.com/products/nxrm3/docs/{0}" target="_blank">{1}-帮助文档。</a>',
    ...
    User_UserAdd_Create_Error: '您没有创建用户的权限',
    User_UserAdd_Create_Success: '创建用户： ',
    ...
    User_UserChangePassword_NoPermission_Error: '您无权更改密码',
